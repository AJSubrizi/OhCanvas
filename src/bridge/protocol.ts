// Shared wire protocol between the React frontend and the Node sidecar.
// Keep this file in sync with sidecar/src/protocol.ts.
...
/** Frontend → sidecar: snapshot of canvas nodes so agents can be canvas-aware. */
export interface CanvasStateMsg {
  type: "canvas_state";
  nodes: CanvasNodeInfo[];
}

/** Frontend → sidecar: a shell node was closed; kill its process. */
...
/** Frontend → sidecar: start/stop the Apple speech-to-text helper. */
...
/** Frontend → sidecar: a natural-language command for the conductor (LLM brain). */
...
/** Conductor agent id — a singleton Pi agent that drives the canvas from commands. */
...
export type ServerMsg =
  ...
  // sidecar → frontend: canvas control commands (driven by agent tools)
  ...
  // sidecar → frontend: a dev-server URL (e.g. vite/next "Local: http://localhost:5173")
  // was detected in the terminal's PTY output. The frontend offers it as a preview.
  ...
  // sidecar → frontend: speech-to-text stream
  | { type: "stt_status"; state: string; message?: string }
  | { type: "stt_partial"; text: string }
  | { type: "stt_final"; text: string }
  // sidecar → frontend: ask the frontend to run the local SmolLM2 conductor
  // (only the frontend can reach the Tauri plugin IPC). Result comes back via
  // `llm_conductor_result`.
  | { type: "llm_conductor_request"; reqId: string; prompt: string };

export const SIDECAR_PORT = 8787;
export const SIDECAR_URL = `ws://127.0.0.1:${SIDECAR_PORT}`;
