...
for (const file of files) {
  ...
  if (file.endsWith(".app.tar.gz") && !file.endsWith(".sig")) {
    ...
    console.log(`Added macOS platform: ${file.includes("aarch64") ? "darwin-aarch64" : "darwin-x86_64"}`);
  }
  // Windows: .msi.zip is the updater archive, .msi.zip.sig is its signature
  if (file.endsWith(".msi.zip") && !file.endsWith(".sig")) {
    const sigFile = `${file}.sig`;
    if (!existsSync(resolve(distDir, sigFile))) {
      console.log(`WARN: missing signature file ${sigFile}, skipping ${file}`);
      continue;
